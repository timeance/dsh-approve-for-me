...
`0.2.1` 已适配 DeepSeek Harness `0.1.0-rc.7`，包括 rc.7 新增的第三方插件 keyed 设置卡片机制。
...
Web 卡片是可选配置入口。rc.7 客户端以 key `approve-for-me` 注册 keyed `settings.plugin.item` slot。卡片只在 loopback 连接中显示，通过插件自己的 loopback-only RPC 读写配置。持久化、schema 校验、revision conflict、脱敏和热加载由 Harness Settings service 负责。卡片不做审批决策，也不依赖 `llm-pi-ai`。
...
| 安装出现 peer warning | 确认 Harness `0.1.0-rc.7` 兼容性，并运行 `pnpm peers check` |
...
rc.7 适配使用 Harness 要求的 keyed 第三方设置卡片注册方式。权限 patch 保留 `Read Only`、`Workspace Write` 和 `Full access`，再添加 `Approve for me`。permission preset 图标仍由 Harness UI 控制。

2026 年 8 月 19 日，`0.2.1` 源码已针对官方 `dsh-v0.1.0-rc.7` tag 的提交 `99f6f02` 完成验证：

- `pnpm peers check`、typecheck、全部 238 项测试和生产构建通过。
- npm pack 生成 26 个文件，包含两份 README。
- 隔离 Web 和 headless Profile 均安装 `0.2.1`，并组合出预期的 preset 与 Host 接线。
- 隔离 Web Host 和插件 client bundle 均返回 HTTP 200；bundle 包含 rc.7 设置 slot 与插件 key。
...
测试 adapter 是临时文件，Git 会忽略它。运行时验证应使用新的隔离 `DSH_HOME`。
